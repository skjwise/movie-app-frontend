...
//     const nowPlayingURL = "https://api.themoviedb.org/3/movie/now_playing?api_key=eacd0d5f3e163cc975fd29fceb3caf04&language=en-US&page=1"
    ...
    //need to add in the Genre List - plus add a click event listener.
...
$(document).ready(function(){
    ...
    var apiKey = 'eacd0d5f3e163cc975fd29fceb3caf04';

    const nowPlayingURL = apiBaseURL + 'movie/now_playing?api_key=' + apiKey;
    const mostPopularURL = "https://api.themoviedb.org/3/movie/popular?api_key=eacd0d5f3e163cc975fd29fceb3caf04&language=en-US&page=1"
		...
		$.getJSON(nowPlayingURL, function(nowPlayingData){
			...
			for(let i = 0; i<nowPlayingData.results.length; i++){
				...
				$.getJSON(thisMovieUrl, function(movieKey){
					...
					var poster = imageBaseUrl+nowPlayingData.results[i].poster_path;
					// console.log(poster);
    ...
    function getMostPopularData(){
		$.getJSON(mostPopularURL, function(mostPopularData){
			// console.log(nowPlayingData);
			//we needed to add .results because nowPlayingData is an array.
			for(let i = 0; i<mostPopularData.results.length; i++){
				// w300 is how wide it is
				var mid = mostPopularData.results[i].id;
				// mid = movie ID
				var thisMovieUrl = apiBaseURL+'movie/'+mid+'/videos?api_key=' + apiKey;
				// console.log(i)
				...
				$.getJSON(thisMovieUrl, function(movieKey){
					...
					var mostPopularHTML = '';
					// added in i to nowPlayingHTML. Without it, only the details for the first movie in the results display in the modal no matter which movie poster you click on.
					mostPopularHTML += '<div class="col-sm-3 eachMovie">';
                    mostPopularHTML += '<button type="button" class="btnModal" data-toggle="modal" data-target="#exampleModal'+ i + '" data-whatever="@' + i + '">'+'<img src="'+poster+'"></button>'; 	
                        mostPopularHTML += '<div class="modal fade" id="exampleModal' + i +'" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">';
                            mostPopularHTML += '<div class="modal-dialog" role="document">';
                                mostPopularHTML += '<div class="modal-content col-sm-12">';
                                    mostPopularHTML += '<div class="col-sm-6 moviePosterInModal">';
                                        mostPopularHTML += '<a href="'+youtubeLink+'"><img src="'+poster+'"></a>'; 
                                        mostPopularHTML += '</div><br>';//close trailerLink
                                        mostPopularHTML += '<div class="col-sm-6 movieDetails">';
										mostPopularHTML += '<div class="movieName">'+title+'</div><br>';
										mostPopularHTML += '<div class="linkToTrailer"><a href="'+youtubeLink+'"><span class="glyphicon glyphicon-play"></span>&nbspPlay trailer</a>' + '</div><br>';	
										mostPopularHTML += '<div class="release">Release Date: '+releaseDate+'</div><br>';
										// mostPopularHTML += '<div class="genre">Genre: '+genre+'</div><br>';
										mostPopularHTML += '<div class="overview">' +overview+ '</div><br>';// Put overview in a separate div to make it easier to style
										mostPopularHTML += '<div class="rating">Rating: '+voteAverage+ '/10</div><br>';
										mostPopularHTML += '<div class="col-sm-3 btn btn-primary">8:30 AM' + '</div>';
										mostPopularHTML += '<div class="col-sm-3 btn btn-primary">10:00 AM' + '</div>';
										mostPopularHTML += '<div class="col-sm-3 btn btn-primary">12:30 PM' + '</div>';
										mostPopularHTML += '<div class="col-sm-3 btn btn-primary">3:00 PM' + '</div>';
										mostPopularHTML += '<div class="col-sm-3 btn btn-primary">4:10 PM' + '</div>';
										mostPopularHTML += '<div class="col-sm-3 btn btn-primary">5:30 PM' + '</div>';
										mostPopularHTML += '<div class="col-sm-3 btn btn-primary">8:00 PM' + '</div>';
										mostPopularHTML += '<div class="col-sm-3 btn btn-primary">10:30 PM' + '</div>';
									mostPopularHTML += '</div>'; //close movieDetails
								mostPopularHTML += '</div>'; //close modal-content
							mostPopularHTML += '</div>'; //close modal-dialog
						mostPopularHTML += '</div>'; //close modal
					mostPopularHTML += '</div>'; //close off each div

					$('#popular-movie-grid').append(mostPopularHTML);
					//Without this line, there is nowhere for the posters and overviews to display so it doesn't show up 
					$('#movieGenreLabel').html("Most Popular");
					//h1 will change depending on what is clicked. Will display "Now Playing" in this case.
				})
			}
		}) 
	...
	function getMoviesByGenre(genre_id){
		...
		$.getJSON(getMoviesByGenreURL, function(genreData){
			...
			for(let i = 0; i<genreData.results.length; i++){
				...
				$.getJSON(thisMovieUrl, function(movieKey){
					var poster = imageBaseUrl+genreData.results[i].poster_path;
	...
	function searchMovies(){
		...
		$.getJSON(searchMovieURL, function(movieSearchResults){
			...
			for (let i = 0; i<movieSearchResults.results.length; i++){
				...
				$.getJSON(thisMovieUrl, function(movieKey){
					// console.log(movieKey)
					var poster = imageBaseUrl+movieSearchResults.results[i].poster_path;
